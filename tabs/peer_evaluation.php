    ...
    try {
        switch ($act) {
            ...
            case 'add_participant':
                ...
                $s = $pdo->prepare("INSERT INTO peer_eval_participants (campaign_id,user_id,username,can_evaluate,can_be_evaluated,added_by) VALUES (?,?,?,?,?,?) ON DUPLICATE KEY UPDATE can_evaluate=VALUES(can_evaluate),can_be_evaluated=VALUES(can_be_evaluated)");
            ...
            case 'save_response':
                ...
                // Confirm current user is evaluator in this campaign
                ...
                if (!$s->fetch()) { echo json_encode(['ok'=>false,'msg'=>'Não é avaliador desta campanha']); exit; }

                $s = $pdo->prepare("INSERT INTO peer_eval_responses (campaign_id,evaluator_id,evaluatee_id,field_key,field_value,is_skip) VALUES (?,?,?,?,?,?) ON DUPLICATE KEY UPDATE field_value=VALUES(field_value),is_skip=VALUES(is_skip)");
...
<script>
const PE_URL = '<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>';
...
function pePost(data, cb) {
    ...
    fetch(PE_URL+'?tab=peer_evaluation', {method:'POST',body:fd})
